...
def create_flashcard_set():
    ...
    red_flags = [
        (
            "AAA rupture red flags",
            "Classic triad: (1) Hypotension (2) Pulsatile abdominal mass (3) Abdominal/back/flank pain → IMMEDIATE vascular surgery, NO CT if unstable",
            "Medicine_Gastroenterology",
            "Medicine/01_GI_Abdominal_Pain_Differentials.html",
        ),
        (
            "Mesenteric ischemia red flag",
            "Severe pain out of proportion to examination + elderly + AF → URGENT CT angiogram, IMMEDIATE surgery consult",
            "Medicine_Gastroenterology",
            "Medicine/01_GI_Abdominal_Pain_Differentials.html",
        ),
        (
            "Ectopic pregnancy red flag",
            "Woman of childbearing age (15-50) with abdominal pain + amenorrhea + PV bleeding → ALWAYS do βhCG",
            "ObGyn",
            "ObGyn/01_Obstetric_History_Differentials.html",
        ),
        (
            "DKA ICU referral criteria",
            "pH <7.1, GCS <12, K+ <3.0 despite replacement, Pregnancy, Age <18 years, Hypotension",
            "Medicine_Endocrinology",
            "Medicine/09_Endocrinology_Diabetes_Management.html",
        ),
        (
            "Ascending cholangitis",
            "Charcot's triad (RUQ pain + fever + jaundice) → URGENT resuscitation, antibiotics, ERCP within 24-48h",
            "Medicine_Gastroenterology",
            "Medicine/01_GI_Abdominal_Pain_Differentials.html",
        ),
        (
        ...
        (
            ...
            "Tracheal deviation away from affected side + hypotension + respiratory distress → IMMEDIATE needle decompression",
            "Medicine_Cardiorespiratory",
            "Medicine/01_Cardiovascular_Respiratory_History.html",
        ),
        (
            "Subarachnoid hemorrhage red flag",
            "Sudden thunderclap headache (worst headache of life) + meningism → URGENT CT head + LP if CT negative",
            "Medicine_Neurology",
            "Medicine/03_Neurology_Headache_Differentials.html",
        ),
        (
            "Temporal arteritis red flag",
            "Age >50 + new headache + jaw claudication + visual symptoms → URGENT ESR/CRP + start prednisolone BEFORE biopsy",
            "Medicine_Neurology",
            "Medicine/03_Neurology_Headache_Differentials.html",
        ),
        (
            "Acute angle-closure glaucoma",
            "Severe eye pain + blurred vision + halos around lights + fixed mid-dilated pupil → URGENT ophthalmology",
            "Medicine_General",
