...
class CitationExtractor:
    ...
    def categorize_claim(self, text: str) -> str:
        ...
        if any(drug in text_lower for drug in ['paracetamol', 'aspirin', 'morphine', 'medication', 'drug', 'mg', 'dose']):
            return "medication"
        elif any(term in text_lower for term in ['bp', 'hr', 'rr', 'spo2', 'vital', 'mmhg']):
            return "vital_signs"
        elif any(term in text_lower for term in ['stemi', 'stroke', 'protocol', 'guideline', 'pathway']):
            return "clinical_guideline"
        elif any(term in text_lower for term in ['diagnostic', 'criteria', 'diagnosis']):
            return "diagnostic_criteria"
        elif any(term in text_lower for term in ['red flag', 'warning', 'emergency', 'contraindication']):
            return "safety"
        elif any(term in text_lower for term in ['examination', 'inspect', 'palpate', 'auscult', 'percuss']):
            return "examination"
        elif any(term in text_lower for term in ['investigation', 'test', 'imaging', 'pathology', 'ecg', 'xray']):
        ...
        try:
            ...
            for i, line in enumerate(lines, start=1):
                ...
                if any(re.search(p, line, re.IGNORECASE) for p in
                       self.CRITICAL_PATTERNS + self.IMPORTANT_PATTERNS + self.MINOR_PATTERNS):
    ...
    def generate_report(self, output_file: str) -> Dict:
        ...
        sorted_claims = sorted(
            self.uncited_claims,
            key=lambda c: (severity_order.get(c.severity, 3), c.file, c.line)
        )
        ...
        stats = {
            ...
            "by_severity": {
                ...
                "important": sum(1 for c in sorted_claims if c.severity == Severity.IMPORTANT.value),
