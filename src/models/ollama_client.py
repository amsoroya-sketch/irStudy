...
class OllamaClient:
    ...
    MODELS = {
        ...
        "deepseek-r1:7b": {
            "description": "DeepSeek reasoning model - best for medical MCQ generation",
            "best_for": [
                "medical_reasoning",
                "complex_reasoning",
                "question_generation",
    ...
    def get_model(
        self, model_name: str, temperature: Optional[float] = None, streaming: bool = False
    ) -> Ollama:
        """
        ...
        return Ollama(
            model=model_name, base_url=self.base_url, temperature=temp, callbacks=callbacks
        )

    def generate(
        self, prompt: str, model_name: str = "meditron:7b", temperature: Optional[float] = None
    ) -> str:
        """
    ...
    def generate_with_template(
        self, template: str, variables: Dict, model_name: str = "meditron:7b"
    ) -> str:
        """
...
def get_validator() -> Ollama:
    ...
    client = OllamaClient()
    return client.get_model("deepseek-r1:7b", temperature=0.3)
...
if __name__ == "__main__":
    ...
    response = client.generate(
        prompt="What are the symptoms of acute coronary syndrome?", model_name="meditron:7b"
    )
