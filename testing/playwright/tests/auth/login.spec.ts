...
test.describe('Login Page', () => {
  ...
  test.describe('Page Structure & Accessibility', () => {
    ...
    test('should have proper ARIA labels and accessibility attributes', async ({ page }) => {
      ...
      // Fill form with valid data to enable submit button
  ...
  test.describe('Invalid Credentials', () => {
    ...
    test('should show error for empty email', async ({ page }) => {
      // Fill email first, then clear it to trigger validation
      await page.fill('input[name="email"]', 'test@example.com');
      await page.fill('input[name="email"]', '');
      await page.fill('input[name="password"]', 'ValidPassword123!');

      // Blur the email field to trigger onBlur validation
      await page.locator('input[name="email"]').blur();

      // Email required error should appear
    ...
    test('should show error for empty password', async ({ page }) => {
      ...
      // Fill password first, then clear it to trigger validation
      await page.fill('input[name="password"]', 'ValidPassword123!');
      ...
      // Password required error should appear
    ...
    test('should show error for incorrect credentials (401)', async ({ page }) => {
      // Note: This test now works after fixing axios interceptor to exclude /auth/login endpoint
      ...
      page.on('request', req => {
        ...
        if (url.includes('/auth/login')) {
          console.log('[TEST] Login request made:', req.postData());
          requestMade = true;
        } else if (url.includes('/auth/refresh')) {
          console.log('[TEST] Refresh request made!');
        } else if (url.includes('/api/')) {
          console.log('[TEST] Other API request:', url);
        }
      });
      ...
      page.on('response', async resp => {
        const url = resp.url();
        if (url.includes('/auth/login')) {
          console.log('[TEST] Login response status:', resp.status());
          console.log('[TEST] Login response body:', await resp.text());
        } else if (url.includes('/auth/refresh')) {
          console.log('[TEST] Refresh response status:', resp.status());
        }
      });
      ...
      if (!requestMade) {
        throw new Error('[TEST] No login request was made!');
      }
      ...
      try {
        ...
        await expect(errorAlert).toContainText(/invalid|credentials|unauthorized|failed/i);
      } catch (e) {
        // If error alert doesn't appear, check submit button state
        const submitButton = page.locator('button[type="submit"]');
        ...
        throw new Error('Error alert did not appear after failed login');
      }
