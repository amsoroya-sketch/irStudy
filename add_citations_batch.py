...
def get_appropriate_citation(claim: str, file_path: str) -> str:
    ...
    if any(drug in claim_lower for drug in ['paracetamol', 'morphine', 'adrenaline', 'insulin', 'heparin', 'warfarin', 'aspirin', 'mg', 'units']):
    ...
    if 'examination' in file_lower or any(term in claim_lower for term in ['inspect', 'palpat', 'percussion', 'auscult', 'jvp', 'murmur']):
...
def add_citations():
    ...
    for claim_data in critical_claims:
        ...
        if line_num <= len(lines):
            ...
            if '(' in original_line and any(src in original_line for src in ['Therapeutic', 'Talley', 'Murtagh', 'AMC', 'eTG']):
                continue
