#!/usr/bin/env python3
"""
Load Sample Data to PostgreSQL Database
Loads MCQs and OSCEs from JSON files to test backend API
"""
...
class DataLoader:
    ...
    def load_mcqs_from_file(self, filepath: str, limit: int = None) -> int:
        ...
        Args:
            ...
            limit: Maximum number of MCQs to load (None = all)

        Returns:
            Number of MCQs loaded
        """
        ...
        for mcq_data in mcqs_data:
            ...
            # Create MCQ object - pass enum objects directly, not .value
            # SQLAlchemy SQLEnum expects enum objects, not string values
            mcq = MCQ(
                ...
                specialty=specialty,  # Pass enum object
                difficulty=difficulty,  # Pass enum object
    ...
    def load_osces_from_file(self, filepath: str, limit: int = None) -> int:
        ...
        Args:
            filepath: Path to OSCE JSON file
            limit: Maximum number of OSCEs to load (None = all)

        Returns:
            Number of OSCEs loaded
        """
        ...
        for osce_data in osces_data:
            ...
            # Map fields to OSCE model
            # candidate_instructions: What the candidate sees (scenario)
            # patient_instructions: Instructions for simulated patient (task descriptions)
            # examiner_instructions: Marking guide (expected answers)
            ...
            # Create OSCE object - pass enum objects directly
            osce = OSCE(
                osce_id=osce_id,
                station_title=station_title,
                station_type=osce_type,  # Pass enum object
                candidate_instructions=candidate_instructions,
                patient_instructions='\n'.join(task_descriptions),
                examiner_instructions=self._format_expected_answers(osce_data.get('expected_answers', {})),
                rubric=rubric,
                specialty=specialty,  # Pass enum object
                difficulty=self._map_difficulty(osce_data.get('difficulty', 'medium')),  # Pass enum object
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Load sample data to PostgreSQL")
    parser.add_argument('--mcqs', type=int, default=50, help='Number of MCQs to load (default: 50)')
    parser.add_argument('--osces', type=int, default=20, help='Number of OSCEs to load (default: 20)')
    ...
    # Load MCQs from high-quality files
    mcq_files = [
    ...
    total_mcqs = 0
    mcqs_per_file = args.mcqs // len(mcq_files)

    for mcq_file in mcq_files:
        filepath = Path(__file__).parent.parent / mcq_file
        if filepath.exists():
            count = loader.load_mcqs_from_file(str(filepath), limit=mcqs_per_file)
            total_mcqs += count
    ...
    total_osces = 0
    osces_per_file = args.osces // len(osce_files)

    for osce_file in osce_files:
        filepath = Path(__file__).parent.parent / osce_file
        if filepath.exists():
            count = loader.load_osces_from_file(str(filepath), limit=osces_per_file)
            total_osces += count
