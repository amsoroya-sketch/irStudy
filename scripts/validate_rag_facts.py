...
class RAGFactValidator:
    ...
    def __init__(self, auto_correct: bool = True, unverified_claims_dir: Optional[Path] = None):
        """
        ...
        self.unverified_claims_dir = unverified_claims_dir or Path(
            "validation_reports/unverified_claims"
        )
        ...
        if claim.claim_type == "dosage":
            ...
            if drug_match:
                ...
                return self.rag.find_correct_dosage(
                    drug_name, claim.context.get("specialty", "general")
                )
            ...
            return self.rag.verify_examination_technique(
                system=claim.context.get("specialty", "general"), technique=claim.claim_text
            )
    ...
    def create_unverified_document(
        self, claim: MedicalClaim, rag_result: RAGVerificationResult
    ) -> str:
        """
        ...
        with open(doc_path, "w", encoding="utf-8") as f:
            ...
            citation_match = re.search(
                r"\([^)]*(?:etg|murtagh|talley|amc|harrison)[^)]*\)",
                claim.original_line,
                re.IGNORECASE,
            )
            if citation_match:
            ...
            for i, source in enumerate(rag_result.sources[:3], 1):
                ...
                f.write(
                    f"{i}. **{source['source']}** p.{source['page']}{australian_flag} [Score: {source['score']:.2f}]\n"
                )
    ...
    def apply_correction(
        self, file_path: Path, line_number: int, original: str, corrected: str
    ) -> bool:
        """
        ...
        for claim in claims:
            ...
            if rag_result.should_correct and rag_result.corrected and self.auto_correct:
                ...
                success = self.apply_correction(
                    file_path, claim.line_number, claim.claim_text, rag_result.corrected
                )
                ...
                issues.append(
                    RAGValidationIssue(
                        claim=claim, rag_result=rag_result, action_taken="verified", severity="info"
                    )
                )
    ...
    def generate_report_markdown(self, output_path: Path):
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            ...
            if self.report.unverified_documents_created:
                f.write(
                    f"## Unverified Claims ({len(self.report.unverified_documents_created)})\n\n"
                )
...
def main():
    ...
    parser.add_argument(
        "--unverified-dir",
        default="validation_reports/unverified_claims",
        help="Unverified claims directory",
    )
    parser.add_argument(
        "--output", default="validation_reports/rag_validation.md", help="Output report path"
    )
    ...
    validator = RAGFactValidator(
        auto_correct=not args.no_auto_correct, unverified_claims_dir=Path(args.unverified_dir)
    )
