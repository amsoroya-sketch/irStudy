...
def extract_red_flags(soup, file_path):
    ...
    red_flag_markers = soup.find_all(
        string=re.compile(r"🚨|RED FLAG|CRITICAL|MUST NOT MISS", re.IGNORECASE)
    )
    ...
    for marker in red_flag_markers[:30]:  # Limit per file
        ...
        if any(
            ...
            for phrase in ["pattern:", "classic triad", "action:", "immediate", "urgent"]
        ):
...
def extract_differentials(soup, file_path):
    ...
    for table in tables[:5]:  # Limit tables per file
        ...
        if any(
            "differential" in h.lower() or "diagnosis" in h.lower() or "causes" in h.lower()
            for h in headers
        ):
    ...
    diff_markers = soup.find_all(
        string=re.compile(r"Differential|Causes of|Consider:", re.IGNORECASE)
    )
...
def extract_physical_exam(soup, file_path):
    ...
    exam_markers = soup.find_all(
        string=re.compile(
            r"Preparation|Position|Permission|Inspection|Palpation|Percussion|Auscultation|5 Ps|IPPA",
            re.IGNORECASE,
        )
    )
    ...
    for marker in exam_markers[:15]:
        ...
        if 3 <= len(items) <= 8:
            ...
            back = "\n".join(
                [f"{i+1}. {li.get_text(strip=True)[:150]}" for i, li in enumerate(items[:6])]
            )
...
def extract_communication(soup, file_path):
    ...
    comm_markers = soup.find_all(
        string=re.compile(r'What to say|SPIKES|Breaking bad news|"', re.IGNORECASE)
    )
...
def extract_australian_context(soup, file_path):
    ...
    aus_markers = soup.find_all(
        string=re.compile(r"eTG|PBS|RACGP|ANZCOR|Australian|Australia|NSW|Medicare", re.IGNORECASE)
    )
    ...
    for marker in aus_markers[:15]:
        ...
        if any(
            term in text for term in ["eTG", "PBS", "Medicare", "Australian guideline", "RACGP"]
        ):
            ...
            if len(sentences) >= 1:
                ...
                cards.append(
                    {"front": front, "back": back, "category": "australian", "difficulty": "medium"}
                )
...
def extract_img_mistakes(soup, file_path):
    ...
    img_sections = soup.find_all(
        string=re.compile(r"Common IMG Mistakes|IMG Common|Mistakes|AVOID", re.IGNORECASE)
    )
    ...
    for section in img_sections[:3]:
        ...
        for item in items[:10]:
            ...
            cards.append(
                {"front": front, "back": back, "category": "img_mistake", "difficulty": "medium"}
            )
...
def main():
    ...
    html_files = [
        f for f in html_files if "MASTER_INDEX" not in str(f) and "START_HERE" not in str(f)
    ]
