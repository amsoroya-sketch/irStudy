...
class QdrantIndexer:
    ...
    def create_collection(self, collection_name: str, vector_size: int = 768):
        ...
        try:
            ...
            logger.info(
                f"✓ Created collection '{collection_name}' with {vector_size}-dimensional vectors"
            )
    ...
    def test_search(
        self,
        collection_name: str = "medical_knowledge",
        query: str = "acute coronary syndrome management",
    ):
...
def main():
    ...
    indexer.upload_embeddings(
        embeddings_file=args.embeddings, collection_name=args.collection, batch_size=args.batch_size
    )
