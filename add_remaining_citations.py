...
def get_citation(claim: str, file_path: str) -> str:
    ...
    if any(d in c for d in ['mg', 'paracetamol', 'morphine', 'insulin', 'heparin', 'warfarin']):
        if 'paed' in f: return "(Therapeutic Guidelines: Paediatric, 2024)"
        if 'surgery' in f: return "(Therapeutic Guidelines: Surgery, 2024)"
        if 'cardio' in f: return "(Therapeutic Guidelines: Cardiovascular, 2024)"
