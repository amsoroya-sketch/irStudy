#!/usr/bin/env python3
"""
Generate Psychiatry MCQs - Day 5 (140 MCQs)
Agent: MED-009 Psychiatry Expert
Topics: Depression (PHQ-9), anxiety (GAD-7), panic disorder, agoraphobia
...
try:
    from agents.medical.med_009_psychiatry import PsychiatryExpert
except ImportError:
    print("ERROR: Could not import PsychiatryExpert from src/agents/medical/med_009_psychiatry.py")
...
DAY = 5
TARGET_COUNT = 140
OUTPUT_FILE = "data-jan-26/mcqs/psychiatry_day5_140_mcqs.json"
LOG_FILE = "data-jan-26/validation/day5_psychiatry_log.txt"

# Topics for Day 5
TOPICS = [
    {"name": "Major Depressive Disorder", "count": 50, "keywords": ["depression", "MDD", "PHQ-9", "antidepressants", "SSRI", "suicidal ideation"]},
    {"name": "Anxiety Disorders", "count": 40, "keywords": ["anxiety", "GAD", "GAD-7", "generalised anxiety", "benzodiazepines"]},
    {"name": "Panic Disorder", "count": 30, "keywords": ["panic disorder", "panic attack", "hyperventilation", "CBT"]},
    {"name": "Agoraphobia", "count": 20, "keywords": ["agoraphobia", "avoidance", "exposure therapy", "social anxiety"]},
]
...
class MCQGenerator:
    ...
    def __init__(self):
        ...
        # Initialize Agent OS
        try:
            self.psychiatry_agent = PsychiatryExpert()
            self.log("✓ Psychiatry Expert Agent (MED-009) loaded")
        except Exception as e:
            self.log(f"ERROR: Could not load Psychiatry Expert: {e}")
            self.psychiatry_agent = None
    ...
    def fetch_rag_citations(self, topic, keywords, retry_count=0):
        ...
        try:
            ...
            results = self.qdrant.search(
                ...
                filter={
                    "must": [
                        {"key": "source", "match": {"any": ["eTG", "RANZCP", "AMH", "PBS", "Beyond Blue"]}}
                    ]
                }
            )
...
1. Create a realistic clinical scenario with:
   ...
   - Presenting complaint (psychiatric symptoms)
   - Duration of symptoms
   - Relevant psychiatric and medical history
   - Mental State Examination findings (appearance, behaviour, mood, affect, thought content, perception, cognition)
   - PHQ-9 or GAD-7 score if depression/anxiety
...
4. Write a comprehensive explanation with:
   ...
   - australian_context: Mention eTG/RANZCP/AMH/PBS/Beyond Blue where relevant
...
7. For psychiatry topics, include relevant:
   - PHQ-9 score (0-27) if depression
   - GAD-7 score (0-21) if anxiety
   - Risk assessment (suicidal ideation, self-harm risk)
   - Medication choices per eTG Psychiatry
   - RANZCP guidelines for treatment

OUTPUT FORMAT (JSON):
{{
    "scenario": "A 35-year-old woman presents with...",
    ...
    "explanation": {{
        ...
        "australian_context": "Per eTG Psychiatry..."
    }},
...
"""
        ...
        try:
            ...
            mcq_data['metadata'] = {
                ...
                "tools_applied": ["PHQ_9_screening", "GAD_7_assessment", "MSE", "risk_assessment"]
            }
    ...
    def validate_mcq_incremental(self, mcq):
        ...
        has_australian_marker = any(marker in full_text_lower for marker in ["etg", "ranzcp", "amh", "pbs", "medicare", "therapeutic guidelines", "beyond blue"])
        if not has_australian_marker:
            errors.append("Missing Australian context markers (eTG/RANZCP/AMH/PBS/Beyond Blue)")
    ...
    def generate_all(self):
        ...
        self.log(f"\n{'='*60}")
        self.log(f"PSYCHIATRY DAY 5 GENERATION (Agent: {AGENT_ID})")
        self.log(f"Target: {TARGET_COUNT} MCQs")
        self.log(f"Output: {OUTPUT_FILE}")
        self.log(f"{'='*60}\n")

        # Pre-generation validation (Gate 1)
        if not self.psychiatry_agent:
            self.log("ERROR: Psychiatry Expert Agent not loaded. Aborting.")
            sys.exit(1)
        if not self.qdrant:
